...
# herdr, macOS only - and the contents say so: gated with mkIf on isDarwin
# (never a conditional import, trap A), so a Linux consumer of the default
# module list gets no herdr config, no migration hook, and - because the
# claude wiring below stays inside the gate - the portable, hook-free
# Claude settings.

{
  config = lib.mkIf pkgs.stdenv.hostPlatform.isDarwin {
    # herdr's Claude integration rides with herdr: this flips the common
    # claude module to ship the authored settings verbatim, SessionStart hook
    # included. The hook keeps its runtime existence guard ([ -x ] on the
    # script herdr itself installs), so a machine that has not run herdr's
    # integration setup simply no-ops.
    dotfiles.claude.herdrHook = true;
